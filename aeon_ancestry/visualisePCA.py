...
def plotReferencePCA(num_loci=128097):
    ...
    for g in pop_order:
        ...
        ax3.scatter(
            gpoints["PC1"] / num_loci, gpoints["PC3"] / num_loci, c=colour_map[g], s=6, label=g
        )
...
def addTrioToPCAplot(coord_df, base_axs):
    ...
    base_axs[2].legend(
        handles=markers, loc="center left", bbox_to_anchor=(1.5, 0.5), title="Samples"
    )
    return base_axs
...
def saveTrioPCAplot(title, case_coords, verbose=True, num_loci=128097):
    ...
    if verbose:
        print(f"saved figure to {title}_PCA_plot.png")
